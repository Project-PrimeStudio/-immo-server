...
app.config['JWT_SECRET_KEY'] = os.environ.get('JWT_SECRET', 'immo-secret-key-2024-change-in-prod')
...
class Apartment(db.Model):
    ...
    type        = db.Column(db.String(20),  nullable=False)   # 'rent' | 'buy'
    ...
    images      = db.Column(db.Text,        nullable=False)   # JSON array of URLs
...
def seed_data():
    if Apartment.query.count() > 0:
        return
    samples = [
        Apartment(title="Bel appartement 3 pièces rénové - Rivoli",
                  ...
                  rooms=3, surface=75,
                  description="Magnifique appartement au cœur de Paris, entièrement rénové, lumineux avec vue sur la rue de Rivoli. Cuisine équipée, parquet ancien, double vitrage."),

        Apartment(title="Studio moderne - Le Marais",
                  ...
                  rooms=1, surface=32,
                  description="Studio moderne dans le quartier du Marais. Cuisine ouverte, salle de bain refaite, proche des transports."),

        Apartment(title="Grand T4 avec terrasse - Montmartre",
                  ...
                  rooms=4, surface=110,
                  description="Superbe appartement avec terrasse de 20m² offrant une vue imprenable sur Paris. Parquet, moulures, cheminée d'époque. Un bien d'exception."),

        Apartment(title="F2 lumineux proche Part-Dieu",
                  ...
                  rooms=2, surface=48,
                  description="Appartement F2 très lumineux proche de la gare Part-Dieu. Balcon avec vue dégagée, cuisine moderne."),

        Apartment(title="Villa vue mer avec piscine",
                  ...
                  rooms=6, surface=200,
                  description="Villa avec vue mer et piscine privée sur la Corniche. 6 pièces, jardin paysagé, garage double. Un bien d'exception."),

        Apartment(title="T3 centre-ville - Nice",
                  type="rent", price=1200, city="Nice",
                  address="3 Avenue Jean Médecin, 06000 Nice",
                  ...
                  rooms=3, surface=65,
                  description="T3 lumineux en plein centre de Nice, à 5 minutes de la Promenade des Anglais. Balcon, parking inclus."),
    ]
    for apt in samples:
        db.session.add(apt)
    db.session.commit()
